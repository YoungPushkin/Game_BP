...
static func get_level_layer_instances(
	...
	for key in layers_dict:
		...
		for index in range(layer_instances.size()-1, -1, -1):
			...
			var layer_type :String = _get_layer_type(world_data, layer_data)
			...
			match match_data:
				...
				{"type": "IntGrid", "tileset": true}:
					...
					layer_created = create_tile_layer(
						tilemap, tile_layer_index, layer_data, options, world_data
					)
					if layer_created:
						tile_layer_index += 1
...
static func create_tile_layer(
	...
	if _get_layer_type(world_data, layer_data) == "Tiles":
...
static func _get_layer_type(world_data: Dictionary, layer_data: Dictionary) -> String:
	var layer_type := str(layer_data.get("__type", layer_data.get("type", "")))
	if not layer_type.is_empty():
		return layer_type

	if not world_data.has("defs"):
		return ""

	for layer_def in world_data.defs.layers:
		if int(layer_def.uid) == int(layer_data.layerDefUid):
			return str(layer_def.type)
